...
interface DeductCutRequest extends Request {
  body: { jobId?: string };
  params: { jobId?: string };
}

export async function deductCut(req: DeductCutRequest, res: Response) {
  ...
  try {
    ...
    const professional = await professionalClient.findOne({
    ...
    const quote = await Quote.findById(job.acceptedQuoteId);
    if (!quote) {
      return res.status(404).send({
        status: "error",
    ...
    // 8. Calculate amounts
    const platformFee = Math.round(quote.price * 0.1 * 100); // in cents
