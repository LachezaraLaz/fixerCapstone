  ...
  const handleForgotPassword = async () => {
    ...
    try {
      const response = await axios.post(
        `https://fixercapstone-production.up.railway.app/reset/requestPasswordReset`,
        { email }
      );
      ...
      if (error.response) {
        setCustomAlertContent({
          ...
          message: error.response.data.error
            ? i18n.t("forgot_password_no_account")
            : i18n.t("forgot_password_failed"),
        });
        setCustomAlertVisible(true);
      } else {
        setCustomAlertContent({
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.title}>
        {i18n.t("forgot_password_no_question_mark")}
      </Text>
      ...
      {isPinSent && (
        <TouchableOpacity
          onPress={() => navigation.navigate("EnterPin", { email })}
        >
