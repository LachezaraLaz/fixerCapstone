...
interface SubmitQuoteRequest extends Request {
  user?: {
    email: string;
  };
  body: {
...
interface GetQuotesByJobRequest extends Request {
  params: {
    jobId: string;
  };
}

interface UpdateQuoteStatusRequest extends Request {
  params: {
    quoteId: string;
  };
  body: {
...
interface GetQuotesByClientEmailRequest extends Request {
  params: {
    clientEmail: string;
  };
}
...
const submitQuote = async (req: SubmitQuoteRequest, res: Response) => {
  ...
  try {
    ...
    const issue = await Job.findById(issueId);
    ...
    logger.info(
      `new quote has been created for issue number:${issueId}. A notification has been sent to the client`
    );
  ...
  try {
    ...
    const populatedQuotes = await Promise.all(
      quotes.map(async (quote) => {
        const professional = await fixerClient.findOne({
          email: quote.professionalEmail,
        });
        if (professional) {
          return {
            ...quote,
...
const updateQuoteStatus = async (
  ...
  try {
    ...
    if (status === QuoteStatus.ACCEPTED) {
      ...
      const existingJob = await Job.findById(quote.issueId);
      ...
      if (existingJob.acceptedQuoteId) {
        return res
          .status(400)
          .json({ message: "This job already has an accepted quote" });
      }

      // Update the status of the associated issue to "in progress"
      await Job.findByIdAndUpdate(quote.issueId, {
      ...
      const professional = await fixerClient.findOne({
        email: updatedQuote.professionalEmail,
      });
      const issue = await Job.findById(updatedQuote.issueId);

      if (!professional || !issue) {
        return res
          .status(404)
          .json({ message: "Professional or issue not found." });
      }
      ...
      const otherProfessionals = await Quote.find({
        issueId: quote.issueId,
        status: QuoteStatus.REJECTED,
      }).populate("professionalEmail");

      for (const rejectedQuote of otherProfessionals) {
        const rejectedProfessional = await fixerClient.findOne({
          email: rejectedQuote.professionalEmail,
        });
      ...
      res.status(200).json({
        message: `Quote accepted, others rejected and job updated to "in progress".`,
        quote: updatedQuote,
      });
      ...
      logger.info(
        `a rejected quote received a notification for issue number ${updatedQuote.issueId}`
      );

      res
        .status(200)
        .json({ message: `Quote rejected successfully.`, quote: updatedQuote });
    }
  } catch (error) {
...
const getQuotesByClientEmail = async (
  ...
  try {
    ...
    const quotes = await Quote.find({ clientEmail }).lean();

    const populated = await Promise.all(
      quotes.map(async (quote) => {
        const pro = await professionalClient
          .findOne({ email: quote.professionalEmail })
          .lean();
