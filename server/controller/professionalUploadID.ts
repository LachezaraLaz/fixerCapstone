 ...
 */
export const professionalUploadID = async (
  req: ProfessionalUploadIDRequest,
  res: Response
) => {
  ...
  try {
    ...
    const user = await professionalClient.findByIdAndUpdate(
      req.user.id,
    ...
    res.json({
      message: "ID image uploaded successfully and form is now complete",
      user,
    });
  } catch (error) {
